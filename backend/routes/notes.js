...
router.post('/addnote', fetchuser, [          //  Add this array...       
    ...
    body('description', 'Description must be atleast 5 characters').isLength({min: 5})
        ...
        try {
            const note = new Note({ 
                ...
                tag: tag,
                user: req.user.id               // 📌📌📌 '_id' ata korle, fetch api a request marar por right side 'Response' a 'user' er id asbe na...
            })
...
router.put("/updatenote/:id", fetchuser, async (req, res) => {
    ...
    try {
        ...
        if ( note.user.toString() !== req.user.id ) {             // 'toString' a user er id dekhabe...
